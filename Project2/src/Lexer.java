...
public class Lexer {
	...
	public Lexer(String filename) throws FileNotFoundException {
		...
		for (String string : new String[] { "cos", "sin", "abs", "radians",
				"degrees", "sqrt" }) {
				...
				case DECIMAL:
					switch (getType(c)) {
						...
						case WHITESPACE:
							temp.type = Token.Type.NUMBER;
							temp.value = Double
									.parseDouble(readChars.toString());
							return temp;
						case DIGIT:
							readChars.append(c);
							break;
	...
	private char getSymbol() {
		if (file.hasNextLine()
				&& (line == null || lineIndex >= line.length())) {
			lineNumber++;
			line = file.nextLine() + '\n';
			lineIndex = 0;
	...
	public Token getToken() {
		...
		try {
			...
			temp = new Token(Token.Type.EOF, lineNumber,
					lineIndex - readChars.length());
		}
		return temp;
	}
